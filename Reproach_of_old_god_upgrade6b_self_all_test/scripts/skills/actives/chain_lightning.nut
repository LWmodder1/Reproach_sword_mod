this.chain_lightning <- this.inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		local ret = [
			...
			},
			{
				...
				text = "By [color=" + this.Const.UI.Color.PositiveValue + "]" + this.SpellHitChance() + "[/color]% chance, the character summon a chain of lightning which inflicts [color=" + this.Const.UI.Color.DamageValue + "]20[/color] - [color=" + this.Const.UI.Color.DamageValue + "]30[/color] damage that ignores armor up to five targets."
			},
			{
				id = 5,
				type = "text",
				icon = "ui/icons/special.png",
				text = "The chance to successfully bring the chain lightning is the sum of 40% of the Fatigue and 60% of the Resolve, and cannot exceed 95%. "  
			},
	...
	function SpellHitChance()
	{
		...
		local ratio1 = 40;
		local ratio2 = 60;
		return this.Math.round(0.01 * ratio1 * actor.getBravery() + 0.01 * ratio2 * actor.getFatigueMax());
	}
	...
	function onUse( _user, _targetTile )
	{
		...
		if ( _user.isAlive() && _targetTile.getEntity().getMoraleState() != 0 && this.Math.rand(1, 100) <= this.Math.min(95, this.SpellHitChance()))
		{
			...
			this.Tactical.EventLog.log("[color=" + this.Const.UI.Color.PositiveValue + "]You successfully summoned the lightning![/color]");
			...
			if (!targetTile.IsEmpty && _targetTile.getEntity().isAlive())
			{
			...
			this.applyEffect(data, 950);		
		}
		else
		{
				this.Tactical.EventLog.log("[color=" + this.Const.UI.Color.NegativeValue + "]You failed to bring the lightning from the sword![/color]");
		}
		return;
	}
