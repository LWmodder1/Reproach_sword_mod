this.parry_lightning <- this.inherit("scripts/skills/skill", {
	m = {
		IsSpent = false,
	},
	function create()
	{
		this.m.ID = "actives.parry_lightning";
	...
	function onUse( _user, _targetTile )
	{
		if (!this.m.IsSpent)
		{
			this.m.Container.add(this.new("scripts/skills/effects/parry_lightning_effect"));
			this.m.IsSpent = true;

			if (!_user.isHiddenToPlayer())
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " uses Lightning Parry");
			}
	...
	function onRemoved()
	{
		this.m.Container.removeByID("effects.parry_lightning");
	}
