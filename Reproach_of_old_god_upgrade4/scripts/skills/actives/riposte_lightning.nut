	...
	function getTooltip()
	{
		...
		if (!this.getContainer().getActor().getCurrentProperties().IsSpecializedInSwords)
		{
			...
			}
			{
				id = 5,
				type = "text",
				text = Inflicts an additional [color=" + this.Const.UI.Color.DamageValue + "]10[/color] - [color=" + this.Const.UI.Color.DamageValue + "]20[/color] damage that ignores armor to a single target for 50% chance. 
			}
