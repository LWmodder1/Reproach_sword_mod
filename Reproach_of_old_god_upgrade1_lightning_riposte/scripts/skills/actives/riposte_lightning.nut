this.riposte_lightning <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Lightning Riposte";
		this.m.Description = "Prepare to immediately counter-attack any opponent that attempts to attack in melee and misses. On a hit, the counter attack will summon lightning that sparks to a single opponent.";
	...
	function getTooltip()
	{
		local ret = [
			...
			},
			{
				id = 4,
				type = "text",
				text = "Inflicts an additional [color=" + this.Const.UI.Color.DamageValue + "]10[/color] - [color=" + this.Const.UI.Color.DamageValue + "]20[/color] damage that ignores armor to a signle target."
			}
		];
	...
	function onUse( _user, _targetTile )
	{
		if (!this.m.IsSpent)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " uses Lightning Riposte");
			}
